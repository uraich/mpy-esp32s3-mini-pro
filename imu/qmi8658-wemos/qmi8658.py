...
class QMI8658():
    ...
    def get_euler_angles(self,gx,gy,gz,ax,ay,az):
        ...
        #/* 在机体坐标系下做向量叉积得到补偿数据 */
        # The compensation data is obtained by performing a
        # vector cross product in the body coordinate system.
        ex = ay*vz - az*vy 
        ey = az*vx - ax*vz 
        ez = ax*vy - ay*vx 
        #/* 对误差进行PI计算，补偿角速度 */
        # Perform PI calculation on the error to compensate for the angular velocity.
        exInt = exInt + ex * Ki
        eyInt = eyInt + ey * Ki
        ezInt = ezInt + ez * Ki
        ...
        gz = gz + Kp*ez + ezInt
        #/* 按照四元素微分公式进行四元素更新 */
        # Update the four elements according to the
        # differential formula of the four elements.
